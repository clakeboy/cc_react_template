import React, { Component, Fragment } from 'react'
import PropTypes from "prop-types"
import './style.css'
import { t } from 'i18next';
interface Props {
    value: string
    width: string
    height: string
    show?: { (): void }
    changeValue?: { (status: string): void }
}

interface State {
    captStatus: string
    popoverVisible: boolean
}

export default class GoCaptchaBtn extends Component<Props, State> {
    static defaultProps = {
        // ============================
        // 为了解决状态冲突加一个 overing
        // ============================
        value: PropTypes.oneOf(['default', 'check', 'error', 'over', 'overing', 'success']),
        width: '300px',
        height: '50px'
    }

    constructor(props: any) {
        super(props);
        this.state = {
            captStatus: 'default',
            popoverVisible: false
        }
    }

    componentDidMount() {
        // $('#go_captcha').popover({
        //   html:true,
        //   content:$('.wg-cap-wrap'),
        //   placement:'top',
        // })
    }

    render() {
        const {captStatus} = this.state
        const {width,height} = this.props
        let style = {
            width,height
        }
        return (
            <>
                <div id='go_captcha' className='wg-cap-btn' style={style}>
                    <div className={(`wg-cap-btn__inner wg-cap-active__${captStatus}`)}>
                        {/* wg-cap-active__default wg-cap-active__error wg-cap-active__over wg-cap-active__success */}
                        <div onClick={this.handleBtnEvent} className="wg-cap-state__default" style={style}>
                            {/*默认状态*/}
                            <div className="wg-cap-state__inner">
                                <div className="wg-cap-btn__ico wg-cap-btn__verify">
                                    <img
                                        src="data:image/svg+xml;base64," alt="" />
                                </div>
                                <span className="wg-cap-btn__text">{t('captcha.default')}</span>
                            </div>
                        </div>
                        <div onClick={this.handleCancelEvent} className="wg-cap-state__check" style={style}>
                            {/* 验证状态 */}
                            <div className="wg-cap-state__inner">
                                <div className="wg-cap-btn__ico">
                                    <img
                                        src="data:image/svg+xml;base64,"
                                        alt="" />
                                </div>
                                <span className="wg-cap-btn__text">{t('captcha.check')}...</span>
                            </div>
                        </div>
                        <div onClick={this.handleBtnEvent} className="wg-cap-state__error" style={style}>
                            {/*验证失败状态*/}
                            <div className="wg-cap-state__inner">
                                <div className="wg-cap-btn__ico">
                                    <img
                                        src="data:image/svg+xml;base64,"
                                        alt="失败" />
                                </div>
                                <span>{t('captcha.error')} <em>{t('captcha.retry')}</em></span>
                            </div>
                        </div>
                        <div onClick={this.handleBtnEvent} className="wg-cap-state__over" style={style}>
                            {/*验证次数过多状态*/}
                            <div className="wg-cap-state__inner">
                                <div className="wg-cap-btn__ico">
                                    <img
                                        src="data:image/svg+xml;base64,"
                                        alt="失败" />
                                </div>
                                <span>{t('captcha.over')} <em>{t('captcha.retry')}</em></span>
                            </div>
                        </div>
                        <div onClick={this.handleCancelEvent} className="wg-cap-state__success" style={style}>
                            {/*验证成功状态*/}
                            <div className="wg-cap-state__inner">
                                <div className="wg-cap-btn__ico">
                                    <img
                                        src="data:image/svg+xml;base64,"
                                        alt="成功" />
                                </div>
                                <span>{t('captcha.success')}</span>
                            </div>
                        </div>
                    </div>
                </div>
            </>
        )
    }

    // ===========
    static getDerivedStateFromProps(nextProps: Props, prevState: State) {
        const res: State = {
            captStatus: '',
            popoverVisible: false
        }
        let count = 0

        if (prevState.popoverVisible) {
            res.captStatus = 'check'
            count++
            // if (prevState.captStatus !== 'check') {
            // nextProps.refresh && nextProps.refresh()
            // }
        } else if (prevState.captStatus === 'check') {
            res.captStatus = nextProps.value
            count++
        }

        if (nextProps.value !== prevState.captStatus) {
            if (prevState.captStatus !== 'check') {
                res.captStatus = nextProps.value
                count++
            }

            // ============================
            // 为了解决状态冲突加一个 overing
            // ============================
            if (nextProps.value === 'overing') {
                res.popoverVisible = false
                res.captStatus = 'over'
                count++
                nextProps.changeValue && nextProps.changeValue('over')

            } else if (nextProps.value === 'success') {
                res['popoverVisible'] = false
                res['captStatus'] = 'success'
                count++
            }
        }

        return (count ? res : null)
    }

    // ================= Methods ================
    handleCancelEvent = (e: React.MouseEvent) => {
        // 阻止合成事件的冒泡
        e.stopPropagation();
        // 阻止与原生事件的冒泡
        e.nativeEvent.stopImmediatePropagation();
        return false
    }
    // handleVisibleChange = (visible) => {
    //     this.setState({ popoverVisible: visible })
    //     if (visible) {
    //         this.props.refresh && this.props.refresh()
    //     }
    // }

    handleBtnEvent = () => {
        // this.setState({
        //   popoverVisible: true
        // })
        this.props.show && this.props.show();
    }

    // handleRefreshEvent = () => {
    //     this.setState({
    //         captStatus: 'check'
    //     })
    //     this.props.refresh && this.props.refresh()
    // }

    // handleConfirmEvent = (data) => {
    //     this.props.confirm && this.props.confirm(data)
    // }

    // handleCloseEvent = () => {
    //     this.setState({
    //         popoverVisible: false
    //     })
    // }
}